...
class Claw4Cfg(LeggedRobotCfg):
    ...
    class init_state(LeggedRobotCfg.init_state):
        pos = [0.0, 0.0, 0.4]#[0.0, 0.0,0.4]#[0.0, 0.0, 3.0]# #[0.0, 0.0, 3.0]  # x,y,z [m]
        rot = [0, 1, 0, 0]
        default_joint_angles = {  # = target angles [rad] when action = 0.0
                'P_base_to_1_Link': 0,
            'P_1_to_2_Link': -0.,
            'P_2_to_left_Link': 0.2,
            'P_2_to_right_Link': 0.2,#GAI
        }

    class control(LeggedRobotCfg.control):
        # PD Drive parameters:
        stiffness = {'P_base_to_1_Link': 70, 'P_1_to_2_Link':120,#刚度大→关节 “执着”→一点偏差就产生大力矩，快速回到目标位置
                     'P_2_to_left_Link': 40.0, 'P_2_to_right_Link': 40.}  # [N*m/rad]
        damping = {'P_base_to_1_Link': 3, 'P_1_to_2_Link':1.5,
                     'P_2_to_left_Link': 3, 'P_2_to_right_Link': 3.}  # [N*m*s/rad]     # [N*m*s/rad]
